...
const MOCK_CLI_PATH = isWindows ? "C:\\mock\\path\\to\\novacode" : "/mock/path/to/novacode"
...
vi.mock("../CliPathResolver", () => ({
	findNovacodeCli: vi.fn().mockResolvedValue({
		cliPath: process.platform === "win32" ? "C:\\mock\\path\\to\\novacode" : "/mock/path/to/novacode",
		shellPath: undefined,
	}),
...
describe("AgentManagerProvider CLI spawning", () => {
	...
	beforeEach(async () => {
		...
		const mockProvider = {
			getState: vi.fn().mockResolvedValue({ apiConfiguration: { apiProvider: "novacode" } }),
		}
		...
		// This ensures findNovacodeCli finds the CLI via local path check (works on all platforms)
		vi.doMock("../../../../utils/fs", () => ({
		...
		vi.doMock("../WorktreeManager", () => ({
			WorktreeManager: vi.fn().mockImplementation(() => ({
				createWorktree: vi.fn().mockResolvedValue({
					...
					path: "/tmp/workspace/.novacode/worktrees/test-branch-123",
					parentBranch: "main",
				}),
	...
	it("adds metadata text for tool requests and skips non chat events", async () => {
		...
		;(provider as any).handleNovacodeEvent(sessionId, {
	...
	it("dedupes repeated events with same ts/type/say/ask", async () => {
		...
		;(provider as any).handleNovacodeEvent(sessionId, { streamEventType: "novacode", payload })
		;(provider as any).handleNovacodeEvent(sessionId, { streamEventType: "novacode", payload })
	...
	it("builds payment required message with parsed title and link", async () => {
		...
		const payload = {
			text: JSON.stringify({
				...
				buyCreditsUrl: "https://nova.ai/billing",
			}),
		}
	...
	describe("parsePaymentRequiredPayload", () => {
		it("parses valid JSON payload with all fields", () => {
			...
			expect(result.buyCreditsUrl).toBe("https://nova.ai/billing")
		})
