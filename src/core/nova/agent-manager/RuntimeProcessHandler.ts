/**
 * RuntimeProcessHandler - Spawns agent processes using @novacode/agent-runtime
 *
...
export interface RuntimeProcessHandlerCallbacks {
	...
	onStartSessionFailed: (
		error?:
			...
			| { type: "api_req_failed"; message: string; payload?: NovacodePayload; authError?: boolean }
			| { type: "payment_required"; message: string; payload?: NovacodePayload },
	) => void
	...
	onPaymentRequiredPrompt?: (payload: NovacodePayload) => void
...
export class RuntimeProcessHandler {
	 ...
	 * - ask:completion_result: Control message for state machine, renders as empty "Nova said"
	 */
	...
	private getProcessEntryPath(): string {
		...
		const devPath = process.env.NOVACODE_DEV_AGENT_RUNTIME_PATH
		...
		try {
			const entryPath = require.resolve("@novacode/agent-runtime/process")
			return entryPath
		} catch {
	...
	private buildAgentConfig(
		...
		if (options?.model) {
			...
			if (config.providerSettings && typeof config.providerSettings === "object") {
				;(config.providerSettings as Record<string, unknown>).novacodeModel = options.model
			}
		}
	...
	private handleAgentReady(proc: ChildProcess, onEvent: (sessionId: string, event: StreamEvent) => void): void {
		...
		if (isResume && sessionData) {
			...
			// This prevents an empty "Nova said" message when we receive the loaded history
		...
		if (extMsg.type === "state" && extMsg.state) {
			...
			if (chatMessages && chatMessages.length > 0) {
				...
				if (!this.sentApiReqStarted.has(sessionId)) {
					...
					this.sentApiReqStarted.add(sessionId)
					const stateEvent: NovacodeStreamEvent = {
						streamEventType: "novacode",
						payload: {
							ts: Date.now(),
							type: "say",
							say: "api_req_started",
