// novacode_change - Nova Gateway server routes
/**
 * Nova Gateway specific routes
 * Handles profile fetching and organization management for Nova Gateway provider
 *
...
import { fetchNovacodeNotifications, NovacodeNotificationSchema } from "../api/notifications.js"
import { KILO_API_BASE, HEADER_FEATURE } from "../api/constants.js" // novacode_change - added HEADER_FEATURE
import { buildKiloHeaders } from "../headers.js" // novacode_change
...
/**
 * Create Nova Gateway routes with OpenCode dependencies injected
 *
 * @example
 * ```typescript
 * import { createKiloRoutes } from "@novacode/nova-gateway"
...
export function createKiloRoutes(deps: KiloRoutesDeps) {
  ...
  return new Hono()
    .get(
      "/profile",
      describeRoute({
        summary: "Get Nova Gateway profile",
        description: "Fetch user profile and organizations from Nova Gateway",
      ...
      async (c: any) => {
        ...
        if (!auth || auth.type !== "oauth") {
          return c.json({ error: "Not authenticated with Nova Gateway" }, 401)
        }
    ...
    .post(
      "/organization",
      describeRoute({
        summary: "Update Nova Gateway organization",
        description: "Switch to a different Nova Gateway organization",
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Proxy a Fill-in-the-Middle completion request to the Nova Gateway",
      ...
      async (c: any) => {
        ...
        const response = await fetch(endpoint, {
          ...
          headers: {
            ...
            // novacode_change start - include kilo headers with autocomplete feature override
            ...buildKiloHeaders(),
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Fetch notifications from Nova Gateway for CLI display",
        ...
        responses: {
          200: {
            ...
            content: {
              "application/json": {
                schema: resolver(z.array(NovacodeNotificationSchema)),
              },
            },
          },
          ...errors(400, 401),
      ...
      async (c: any) => {
        ...
        const notifications = await fetchNovacodeNotifications({
          novacodeToken: token,
          novacodeOrganizationId: organizationId,
        })
