...
import type { NovacodeProfile, Organization, NovacodeBalance } from "../types.js"
import { KILO_API_BASE, DEFAULT_MODEL, DEFAULT_FREE_MODEL } from "./constants.js"

/**
 * Fetch user profile from Kilo API
 */
export async function fetchProfile(token: string): Promise<NovacodeProfile> {
  ...
  // Transform to flat NovacodeProfile structure
  return {
 ...
 */
export async function fetchBalance(token: string, organizationId?: string): Promise<NovacodeBalance | null> {
  try {
    ...
    if (organizationId) {
      headers["x-novacode-organizationid"] = organizationId
    }
...
export async function fetchProfileWithBalance(token: string): Promise<{
  profile: NovacodeProfile
  balance: NovacodeBalance | null
}> {
