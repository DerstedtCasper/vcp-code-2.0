 ...
 */
export const NovacodeNotificationSchema = z.object({
  id: z.string(),
  title: z.string(),
  message: z.string(),
  action: z
...
export type NovacodeNotification = z.infer<typeof NovacodeNotificationSchema>

const NotificationsResponseSchema = z.object({
  notifications: z.array(NovacodeNotificationSchema),
})
 ...
 */
export async function fetchNovacodeNotifications(options: {
  novacodeToken?: string
  novacodeOrganizationId?: string
}): Promise<NovacodeNotification[]> {
  const token = options.novacodeToken
  if (!token) return []
