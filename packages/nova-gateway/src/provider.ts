...
import type { NovaProviderOptions } from "./types.js"
...
/**
 * Create a NovaCode provider instance
 *
 * This provider wraps the OpenRouter SDK with NovaCode-specific configuration
 * including custom authentication, headers, and base URL.
 *
 * @example
 * ```typescript
 * const provider = createKilo({
 *   novacodeToken: "your-token-here",
 ...
 * ```
 */
export function createKilo(options: NovaProviderOptions = {}): SDK {
  ...
  const customHeaders = {
    ...
    ...buildKiloHeaders(undefined, {
      novacodeOrganizationId: options.novacodeOrganizationId,
      novacodeTesterWarningsDisabledUntil: undefined,
    }),
    ...options.headers,
  }
  ...
  // Create OpenRouter provider with NovaCode configuration
