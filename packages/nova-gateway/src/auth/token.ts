...
/**
 * Validate NovaCode token format
 */
export function isValidNovacodeToken(token: string): boolean {
 ...
 */
export function getApiKey(options: { novacodeToken?: string; apiKey?: string }): string | undefined {
  return options.novacodeToken ?? options.apiKey
}
