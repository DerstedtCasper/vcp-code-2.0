...
export const ServerProvider: ParentComponent = (props) => {
  ...
  onMount(() => {
    const unsubscribe = vscode.onMessage((message: ExtensionMessage) => {
      switch (message.type) {
        case "ready":
          console.log("[VCP] Server ready:", message.serverInfo)
        ...
        case "connectionState":
          console.log("[VCP] Connection state changed:", message.state)
        ...
        case "error":
          console.error("[VCP] Server error:", message.message)
          setError(message.message)
          break

        case "profileData":
          console.log("[VCP] Profile data:", message.data ? "received" : "null")
          setProfileData(message.data)
          break

        case "deviceAuthStarted":
          console.log("[VCP] Device auth started")
          setDeviceAuth({
            status: "pending",
            code: message.code,
        ...
        case "deviceAuthComplete":
          console.log("[VCP] Device auth complete")
        ...
        case "deviceAuthFailed":
          console.log("[VCP] Device auth failed:", message.error)
          setDeviceAuth({ status: "error", error: message.error })
          break

        case "deviceAuthCancelled":
          console.log("[VCP] Device auth cancelled")
