...
VCP Code is an open source AI coding agent platform. It ships as multiple products that all build on the same backend. This package (`packages/VCP-vscode/`) is the **VS Code extension** — one of several clients.

### Products and How They Relate

All products are thin clients over the **CLI** (`packages/opencode/`, published as `@kilocode/cli`). The CLI is a fork of upstream [OpenCode](https://github.com/anomalyco/opencode) with VCP-specific additions (gateway auth, telemetry, migration, code review, branding). It contains the full AI agent runtime, tool execution, session management, provider integrations (500+ models), and an HTTP API server.

Every client spawns or connects to a `VCP serve` process and communicates via HTTP REST + SSE using the auto-generated `@kilocode/sdk`.
...
| -------------------------- | -------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------- |
| VCP CLI (TUI)             | `packages/opencode/`                   | Interactive terminal UI (SolidJS + OpenTUI)         | In-process — TUI and server run together                          |
| VCP CLI (`VCP run`)      | `packages/opencode/`                   | Non-interactive headless mode for scripting         | In-process — no network socket                                    |
| **VCP VS Code Extension** | **`packages/VCP-vscode/`**            | VS Code extension with sidebar chat + Agent Manager | Bundles CLI binary, spawns `VCP serve --port 0` as child process |
| OpenCode Desktop           | `packages/desktop/`                    | Standalone native app (Tauri)                       | Bundles CLI binary as sidecar, spawns `VCP serve`                |
| OpenCode Web (`VCP web`)  | `packages/opencode/` + `packages/app/` | Browser-based UI                                    | CLI starts server + opens browser                                 |

**OpenCode Desktop** (`packages/desktop/`) and `VCP web` both render the shared `@opencode-ai/app` SolidJS frontend (`packages/app/`). They differ only in the platform layer (Tauri native APIs vs browser APIs). Neither has any relationship to this extension or the Agent Manager — they are independent clients of the same `VCP serve` backend.

### VCP-Domain Packages

| Package                    | Name                       | Role                                                                                                                         |
| -------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `packages/VCP-vscode/`    | `VCP-code`                | **This package.** VS Code extension.                                                                                         |
| `packages/VCP-gateway/`   | `@kilocode/VCP-gateway`   | Auth (device flow), AI provider routing (OpenRouter), VCP API integration (profile, balance, teams)                         |
| `packages/VCP-ui/`        | `@kilocode/kilo-ui`        | SolidJS component library (40+ components, built on `@kobalte/core`). Shared by this extension's webview and `packages/app/` |
| `packages/VCP-telemetry/` | `@kilocode/VCP-telemetry` | PostHog analytics + OpenTelemetry tracing for the CLI                                                                        |
| `packages/VCP-i18n/`      | `@kilocode/kilo-i18n`      | Translation strings (16 languages)                                                                                           |
| `packages/VCP-docs/`      | `@kilocode/VCP-docs`      | Documentation site (Next.js + Markdoc)                                                                                       |

### Upstream OpenCode Packages (not VCP-specific)
...
| `packages/app/`      | `@opencode-ai/app`     | Shared SolidJS web UI consumed by desktop app and `VCP web`                             |
...
The extension bundles its own CLI binary at `bin/VCP` — it does NOT use a system-installed CLI. To build it:
...
The script checks for a prebuilt binary in `packages/opencode/dist/`, builds the CLI if needed, and copies it to `bin/VCP`.

## Architecture

### Extension ↔ CLI Backend

The extension is a client of the CLI. At startup it spawns `bin/VCP serve --port 0`, captures the dynamically-assigned port from stdout, and communicates over HTTP + SSE. A random password is generated and passed via `KILO_SERVER_PASSWORD` env var for basic auth.

```
Extension (Node.js)                          CLI Backend (child process)
┌──────────────────────────┐                ┌──────────────────────┐
│ KiloConnectionService    │── HTTP/SSE ──> │ VCP serve --port 0  │
...
- HTML root has `data-theme="VCP-vscode"` to activate VCP-ui's VS Code theme bridge
...
All Agent Manager sessions share the **single `VCP serve` process** managed by `KiloConnectionService`. No separate server is spawned per session. Session isolation comes from directory scoping — worktree sessions pass the worktree path to the CLI backend, which creates a session scoped to that directory.
...
- [`chat.css`](webview-ui/src/styles/chat.css) is being progressively migrated — when replacing a component with VCP-ui, remove the corresponding CSS rules from it
- New CSS for components not yet in VCP-ui goes into `chat.css` grouped by comment-delimited sections (`/* Component Name */`). Once a VCP-ui equivalent exists, remove the section.
- **Check the desktop app first**: [`packages/app/src/`](../../packages/app/src/) is the reference implementation for how VCP-ui components are composed together. Always check how the app uses a component before implementing it in the webview — don't just look at the component API in isolation.
- **`data-component` and `data-slot` attributes carry CSS styling** — VCP-ui uses `[data-component]` and `[data-slot]` attribute selectors, not class names. When the app uses e.g. `data-component="permission-prompt"` and `data-slot="permission-actions"`, these get VCP-ui styling for free.
- **Prefer VCP-ui styles**: Always reuse existing VCP-ui CSS variables, tokens, and component styles instead of writing custom CSS. If a style doesn't exist in VCP-ui yet, add it there and reuse it rather than inlining or duplicating styles in the webview.
- **Icons**: VCP-ui has 75+ custom SVG icons in [`packages/ui/src/components/icon.tsx`](../../packages/ui/src/components/icon.tsx). To list all available icon names: `node -e "const c=require('fs').readFileSync('../../packages/ui/src/components/icon.tsx','utf8');[...c.matchAll(/^\\s{2}[\"']?([\\w-]+)[\"']?:\\s*\x60/gm)].map(m=>m[1]).sort().forEach(n=>console.log(n))"`. Icon names use both hyphenated (`arrow-left`) and bare-word (`brain`, `console`, `providers`) keys.
...
- All debug output must be prepended with `[VCP New]` for easy filtering

## Naming Conventions

- All VSCode commands must use `VCP-code.new.` prefix (not `VCP-code.`)
- All view IDs must use `VCP-code.new.` prefix (e.g., `VCP-code.new.sidebarView`)
...
This package is entirely VCP-specific — `kilocode_change` markers are NOT needed in any files under `packages/VCP-vscode/`. The markers are only necessary when modifying shared upstream opencode files.
