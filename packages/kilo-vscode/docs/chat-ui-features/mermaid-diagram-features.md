...
- The **"Fix with AI"** action currently relies on VCP-side AI plumbing. With the agent runtime moving to VCP CLI per [`docs/opencode-core/opencode-migration-plan.md`](docs/opencode-core/opencode-migration-plan.md:1), you likely need to re-route this button to:
  - either a dedicated VCP CLI prompt/tool that returns corrected Mermaid source, or
  - a small VCP-side helper that asks VCP CLI to fix the snippet (so the button remains functional without the legacy VCP orchestration loop).
- VCP CLI UI doesn’t appear to ship an equivalent Mermaid renderer/fixer (only a Mermaid file icon is present in [`packages/ui/src/components/file-icons/types.ts`](https://github.com/DerstedtCasper/vcp-code-2.0/blob/main/packages/ui/src/components/file-icons/types.ts:1)).
