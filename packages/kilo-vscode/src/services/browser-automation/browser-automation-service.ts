...
export class BrowserAutomationService implements vscode.Disposable {
  ...
  private static readonly MCP_SERVER_NAME = "VCP-playwright"

  constructor(private readonly connectionService: KiloConnectionService) {
    // Listen for settings changes
    this.disposables.push(
      vscode.workspace.onDidChangeConfiguration((e) => {
        if (e.affectsConfiguration("vcp-code.new.browserAutomation")) {
  ...
  private async register(): Promise<void> {
    ...
    if (!httpClient) {
      console.error("[VCP] BrowserAutomationService: No HTTP client available")
    ...
    try {
      ...
      if (serverStatus?.status === "connected") {
        ...
        console.error(
          "[VCP] BrowserAutomationService: MCP server failed:",
      ...
      console.error("[VCP] BrowserAutomationService: Failed to register MCP server:", error)
      this.setState("failed")
    }
  }
  ...
  private async unregister(): Promise<void> {
    ...
    if (httpClient) {
      try {
        ...
        console.error("[VCP] BrowserAutomationService: Failed to disconnect MCP server:", error)
      }
    }
  ...
  private setState(state: BrowserAutomationState): void {
    ...
    console.log(`[VCP] BrowserAutomationService: State ${this.state} → ${state}`)
    this.state = state
