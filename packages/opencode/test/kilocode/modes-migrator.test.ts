...
import { ModesMigrator } from "../../src/novacode/modes-migrator"
...
describe("ModesMigrator", () => {
  ...
  describe("convertPermissions", () => {
    ...
    test("converts fileRegex groups to restricted permissions", () => {
      const groups: ModesMigrator.NovacodeMode["groups"] = [
        "read",
    ...
    test("handles tuple without fileRegex", () => {
      const groups: ModesMigrator.NovacodeMode["groups"] = [["edit", {}]]
      const permissions = ModesMigrator.convertPermissions(groups)
  ...
  describe("convertMode", () => {
    test("converts full mode to agent config", () => {
      const mode: ModesMigrator.NovacodeMode = {
        slug: "translate",
        name: "Translate",
  ...
  - slug: translate
    ...
    groups:
      - read`,
          ...
          // Create project .novacodemodes (should win)
          await Bun.write(
            path.join(dir, ".novacodemodes"),
            `customModes:
  - slug: translate
