...
import { NovacodeConfigInjector } from "../../src/novacode/config-injector"
import { tmpdir } from "../fixture/fixture"
import path from "path"
import fs from "fs/promises"

describe("NovacodeConfigInjector", () => {
  describe("buildConfig", () => {
    test("returns empty config when no modes exist", async () => {
      await using tmp = await tmpdir()

      const result = await NovacodeConfigInjector.buildConfig({ projectDir: tmp.path, skipGlobalPaths: true })
  ...
  - slug: code
    ...
    test("includes workflows as commands in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".novacode", "workflows")
          await Bun.write(
  ...
  - slug: translate
    ...
    // novacode_change start - Rules migration tests
    test("includes rules in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await fs.mkdir(path.join(dir, ".novacode", "rules"), { recursive: true })
          await Bun.write(path.join(dir, ".novacode", "rules", "main.md"), "# Rules")
        },
      })

      const result = await NovacodeConfigInjector.buildConfig({
        projectDir: tmp.path,
  ...
  - slug: translate
    ...
    // novacode_change start - Ignore migration tests
    test("includes ignore patterns in config", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".novacodeignore"), "secrets/\n*.env")
        },
      })
  ...
  - slug: translate
    ...
    test("handles negation patterns in ignore file", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".novacodeignore"), "*.env\n!.env.example")
        },
      })
  ...
  describe("getEnvVars", () => {
    test("returns empty object for empty config", () => {
      const envVars = NovacodeConfigInjector.getEnvVars("{}")
    ...
    test("returns KILO_CONFIG_CONTENT for non-empty config", () => {
      ...
      const envVars = NovacodeConfigInjector.getEnvVars(config)
