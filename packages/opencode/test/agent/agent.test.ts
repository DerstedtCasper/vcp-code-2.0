...
test("returns default native agents when no config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(names).toContain("code") // novacode_change
      expect(names).toContain("plan")
      expect(names).toContain("debug") // novacode_change
      expect(names).toContain("orchestrator") // novacode_change
      expect(names).toContain("ask") // novacode_change
...
// novacode_change start - ask agent tests
...
test("agent steps/maxSteps config sets steps property", async () => {
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const code = await Agent.get("code") // novacode_change
      const plan = await Agent.get("plan")
      expect(code?.steps).toBe(50) // novacode_change
...
test("agent name can be overridden", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        code: { name: "Coder" }, // novacode_change
      },
    },
  })
  await Instance.provide({
...
test("agent prompt can be set from config", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        code: { prompt: "Custom system prompt" }, // novacode_change
      },
    },
  })
  await Instance.provide({
...
test("defaultAgent returns build when no default_agent config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(agent).toBe("code") // novacode_change
    },
  })
})
...
test("defaultAgent returns plan when code is disabled and default_agent not set", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      const agent = await Agent.defaultAgent()
      // novacode_change - code is disabled, so it should return plan (next primary agent)
      expect(agent).toBe("plan")
    },
  })
})
...
test("defaultAgent throws when all primary agents are disabled", async () => {
  await using tmp = await tmpdir({
    config: {
      agent: {
        // novacode_change start - disable all primary agents
        code: { disable: true },
        plan: { disable: true },
        debug: { disable: true },
  ...
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // novacode_change - all primary agents are disabled
      await expect(Agent.defaultAgent()).rejects.toThrow("no primary visible agent found")
    },
  })
})

// novacode_change start - Backward compatibility tests for "build" -> "code" rename
