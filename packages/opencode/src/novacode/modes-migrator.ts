...
export namespace ModesMigrator {
  // Novacode mode structure
  export interface NovacodeMode {
    slug: string
    name: string
  ...
  export interface NovacodeModesFile {
    customModes: NovacodeMode[]
  }

  // Default modes to skip - these have native Opencode equivalents
  // novacode_change - added "build" for backward compatibility after renaming "build" to "code"
  ...
  export function convertPermissions(groups: NovacodeMode["groups"]): Config.Permission {
  ...
  export function convertMode(mode: NovacodeMode): Config.Agent {
  ...
  export async function readModesFile(filepath: string): Promise<NovacodeMode[]> {
    try {
      const content = await fs.readFile(filepath, "utf-8")
      // Wrap YAML content in frontmatter delimiters so gray-matter can parse it
      const wrapped = `---\n${content}\n---`
      const parsed = matter(wrapped).data as NovacodeModesFile
  ...
  export async function migrate(options: {
    ...
    if (!options.skipGlobalPaths) {
      ...
      const cliGlobalPath = path.join(os.homedir(), ".novacode", "cli", "global", "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(cliGlobalPath)))

      // 3. Home directory .novacodemodes
      const homeModesPath = path.join(os.homedir(), ".novacodemodes")
    ...
    // 5. Project .novacodemodes
    const projectModesPath = path.join(options.projectDir, ".novacodemodes")
    allModes.push(...(await readModesFile(projectModesPath)))

    // Deduplicate by slug (later entries win)
    const modesBySlug = new Map<string, NovacodeMode>()
