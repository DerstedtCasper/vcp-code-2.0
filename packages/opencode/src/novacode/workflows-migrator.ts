...
export namespace WorkflowsMigrator {
  const KILOCODE_WORKFLOWS_DIR = ".novacode/workflows"
  const GLOBAL_WORKFLOWS_DIR = path.join(os.homedir(), ".novacode", "workflows")

  export interface NovacodeWorkflow {
    name: string
    path: string
    content: string
  ...
  async function loadWorkflowsFromDir(dir: string, source: "global" | "project"): Promise<NovacodeWorkflow[]> {
    if (!(await Filesystem.isDir(dir))) return []
    const files = await findWorkflowFiles(dir)
    const workflows: NovacodeWorkflow[] = []
  ...
  export async function discoverWorkflows(projectDir: string, skipGlobalPaths?: boolean): Promise<NovacodeWorkflow[]> {
    const workflows: NovacodeWorkflow[] = []
    ...
    if (!skipGlobalPaths) {
      ...
      // 2. Home directory ~/.novacode/workflows (fallback/alternative location)
      workflows.push(...(await loadWorkflowsFromDir(GLOBAL_WORKFLOWS_DIR, "global")))
    }

    // 3. Project workflows (.novacode/workflows/)
  ...
  export function convertToCommand(workflow: NovacodeWorkflow): Config.Command {
    return {
  ...
  export async function migrate(options: {
    ...
    const workflowsByName = new Map<string, NovacodeWorkflow>()
