...
This document explains how Novacode configurations are automatically migrated to Opencode.
...
This section explains how Novacode custom modes are automatically migrated to Opencode agents.

## Overview

Novacode stores custom modes in YAML files. When Opencode starts, it reads these files and converts them to Opencode's agent format, injecting them via the `OPENCODE_CONFIG_CONTENT` mechanism.
...
| Platform | Preferred Path                                                                                      | Legacy Path                                                                                          |
| -------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| macOS    | `~/Library/Application Support/Code/User/globalStorage/vcpcode.vcp-code/settings/custom_modes.yaml` | `~/Library/Application Support/Code/User/globalStorage/novacode.kilo-code/settings/custom_modes.yaml` |
| Windows  | `%APPDATA%/Code/User/globalStorage/vcpcode.vcp-code/settings/custom_modes.yaml`                     | `%APPDATA%/Code/User/globalStorage/novacode.kilo-code/settings/custom_modes.yaml`                     |
| Linux    | `~/.config/Code/User/globalStorage/vcpcode.vcp-code/settings/custom_modes.yaml`                     | `~/.config/Code/User/globalStorage/novacode.kilo-code/settings/custom_modes.yaml`                     |

### Project Modes

| Location         | Description                                  |
| ---------------- | -------------------------------------------- |
| `.novacodemodes` | Project-specific modes in the workspace root |

## Field Mapping

### Migrated Fields

| Novacode Field       | Opencode Field | Notes                                                 |
| -------------------- | -------------- | ----------------------------------------------------- |
...
Novacode uses "groups" to define what tools a mode can access. These are converted to Opencode's permission system:

| Novacode Group | Opencode Permission | Notes                      |
| -------------- | ------------------- | -------------------------- |
...
Novacode supports restricting edit access to specific file patterns:
...
The following Novacode default modes are **skipped** during migration because Opencode has native equivalents:

| Novacode Mode  | Reason                                                   |
| -------------- | -------------------------------------------------------- |
...
The following Novacode features are not yet migrated:

| Feature                            | Status      | Notes                                |
| ---------------------------------- | ----------- | ------------------------------------ |
| Rules (`.novacode/rules/`)         | Phase 2     | Will map to `instructions` array     |
| Workflows (`.novacode/workflows/`) | Phase 2     | Will map to custom commands          |
...
Novacode skills are automatically discovered and made available in Opencode. This is **not a migration** - skills remain in their original locations and can be managed independently by the Kilo VSCode extension.

## How It Works

Opencode scans additional directories for skills alongside its native `.opencode/skill/` locations. The `NovacodePaths.skillDirectories()` function provides these paths.
...
The scanner walks up from the current directory to the git worktree root, finding all `.novacode/skills/` directories:
...
| Platform | Preferred Path                                                                    | Legacy Path                                                                                        |
| -------- | --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| macOS    | `~/Library/Application Support/Code/User/globalStorage/vcpcode.vcp-code/skills/` | `~/Library/Application Support/Code/User/globalStorage/novacode.kilo-code/skills/`                 |
| Windows  | `%APPDATA%/Code/User/globalStorage/vcpcode.vcp-code/skills/`                     | `%APPDATA%/Code/User/globalStorage/novacode.kilo-code/skills/`                                     |
| Linux    | `~/.config/Code/User/globalStorage/vcpcode.vcp-code/skills/`                     | `~/.config/Code/User/globalStorage/novacode.kilo-code/skills/`                                     |
...
2. `.novacode/skills/` (walk-up)
3. `~/.novacode/skills/`
4. VSCode extension storage
5. `.opencode/skill/` (walk-up)
6. `~/.opencode/skill/` (highest priority)

This means Opencode native skills take precedence over Novacode skills with the same name.

## Mode-Specific Skills

Novacode supports mode-specific skills in `skills-{mode}/` directories (e.g., `skills-code/`, `skills-architect/`). These are **not currently migrated** to Opencode.
...
.novacode/skills/shared-skill -> ...  # Symlink
...
Novacode rules are migrated to Opencode's `instructions` array. See [`rules-migrator.ts`](../rules-migrator.ts).

## Source Locations

| Location                      | Description                   |
| ----------------------------- | ----------------------------- |
| `.novacoderules`              | Legacy project rules file     |
| `.novacode/rules/*.md`        | Project rules directory       |
| `~/.novacode/rules/*.md`      | Global rules directory        |
| `.novacoderules-{mode}`       | Mode-specific legacy rules    |
| `.novacode/rules-{mode}/*.md` | Mode-specific rules directory |

---

# Workflows Migration

Novacode workflows are migrated to Opencode commands. See [`workflows-migrator.ts`](../workflows-migrator.ts).

## Source Locations

| Location                     | Description                     |
| ---------------------------- | ------------------------------- |
| `.novacode/workflows/*.md`   | Project workflows               |
| `~/.novacode/workflows/*.md` | Global workflows                |
...
Novacode MCP server configurations are migrated to Opencode's `mcp` config. See [`mcp-migrator.ts`](../mcp-migrator.ts).
...
## Source Location (migration from Novacode)
...
When connected to Nova Gateway, the CLI fetches and displays notifications from the Kilo API. This allows Kilo to communicate important announcements, feature updates, and tips to users.

## How It Works

1. **On startup**, if the user is authenticated with Nova Gateway, the CLI fetches notifications from `https://api.kilo.ai/api/users/notifications`
...
```typescript
interface NovacodeNotification {
...
| ------------------------- | ------------------- |
| Connected to Nova Gateway | Yes                 |
| Not connected to Kilo     | No                  |
| No notifications from API | No                  |

## Related Files

- [`notifications.ts`](../../../../nova-gateway/src/api/notifications.ts) - Fetch function and types
- [`routes.ts`](../../../../nova-gateway/src/server/routes.ts) - Server endpoint `/kilo/notifications`
