...
import { NovacodePaths } from "./paths"

export namespace McpMigrator {
  const log = Log.create({ service: "novacode.mcp-migrator" })

  // Remote transport types used by the Novacode extension
  const REMOTE_TYPES = new Set(["streamable-http", "sse"])

  function isRemote(server: NovacodeMcpServer): boolean {
    return !!server.type && REMOTE_TYPES.has(server.type)
  }

  // Novacode MCP server structure
  export interface NovacodeMcpServer {
    command?: string
    args?: string[]
  ...
  export interface NovacodeMcpSettings {
    mcpServers: Record<string, NovacodeMcpServer>
  }
  ...
  export async function readMcpSettings(filepath: string): Promise<NovacodeMcpSettings | null> {
    if (!(await Filesystem.exists(filepath))) return null

    const content = await fs.readFile(filepath, "utf-8")
    return JSON.parse(content) as NovacodeMcpSettings
  }

  export function convertServer(name: string, server: NovacodeMcpServer): Config.Mcp | null {
  ...
  export async function migrate(options?: {
    ...
    const allServers: Array<{ name: string; server: NovacodeMcpServer }> = []

    if (!options?.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global MCP settings)
      for (const storage of NovacodePaths.vscodeGlobalStorages()) {
        const vscodeSettingsPath = path.join(storage, "settings", "mcp_settings.json")
    ...
    // The Novacode extension uses ".novacode/mcp.json" for project-level settings
    // (not "mcp_settings.json" which is only used for global settings)
    if (options?.projectDir) {
      const projectSettingsPath = path.join(options.projectDir, ".novacode", "mcp.json")
    ...
    const serversByName = new Map<string, NovacodeMcpServer>()
  ...
  /**
   * Load Novacode MCP servers and return them as an opencode config partial.
  ...
  export async function loadMcpConfig(projectDir: string): Promise<Record<string, Config.Mcp>> {
    try {
      ...
      if (Object.keys(result.mcp).length > 0) {
        log.debug("loaded novacode MCP servers", {
      ...
      for (const skipped of result.skipped) {
        log.debug("skipped novacode MCP server", { name: skipped.name, reason: skipped.reason })
      }

      for (const warning of result.warnings) {
        log.warn("novacode MCP migration warning", { warning })
      }

      return result.mcp
    } catch (err) {
      log.warn("failed to load novacode MCP servers", { error: err })
      return {}
    }
  }
}
