...
export namespace NovacodePaths {
  const VSCODE_EXTENSION_IDS = ["vcpcode.vcp-code", "novacode.kilo-code"] as const
   ...
   * Ordered by preference: vcpcode.vcp-code, then novacode.kilo-code.
   */
  ...
  /** Global Novacode directory in user home: ~/.novacode */
  export function globalDir(): string {
    return path.join(os.homedir(), ".novacode")
  }

  /**
   * Discover Novacode directories containing skills.
   * Returns parent directories (.novacode/) for glob pattern "skills/[*]/SKILL.md".
   *
   * - Walks up from projectDir to worktreeRoot for .novacode/
   * - Includes global ~/.novacode/
  ...
  export async function skillDirectories(opts: {
    ...
    // 1. Walk up from project dir to worktree root for .novacode/
    // Returns .novacode/ directories (not .novacode/skills/) because
    ...
    for (const dir of projectDirs) {
      ...
      if (await Filesystem.isDir(skillsDir)) {
        directories.push(dir) // Return parent (.novacode/), not skills/
      }
    }
