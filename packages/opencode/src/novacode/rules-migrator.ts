...
export namespace RulesMigrator {
  // Only support .novacoderules (no migration for .roorules or .clinerules)
  const LEGACY_RULE_FILE = ".novacoderules"

  // Directory-based rules
  const KILOCODE_RULES_DIR = ".novacode/rules"
  const GLOBAL_RULES_DIR = path.join(os.homedir(), ".novacode", "rules")
  ...
  export async function discoverRules(projectDir: string): Promise<RuleFile[]> {
    ...
    // 1. Global rules directory (~/.novacode/rules/*.md)
    ...
    // 2. Project .novacode/rules/ directory
    ...
    // 3. Legacy .novacoderules file (only novacode, not roo/cline)
    ...
    for (const mode of KNOWN_MODES) {
      // Mode-specific directory (.novacode/rules-{mode}/*.md)
      const modeDir = path.join(projectDir, `.novacode/rules-${mode}`)
      ...
      // Legacy mode-specific file (.novacoderules-{mode})
      const legacyModeFile = path.join(projectDir, `.novacoderules-${mode}`)
      ...
      if (rule.source === "legacy") {
        warnings.push(
          `Legacy rule file '${path.basename(rule.path)}' found. ` +
            `Consider migrating to .novacode/rules/ directory.`,
        )
      }
    }
