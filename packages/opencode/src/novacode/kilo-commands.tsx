/**
 * Nova Gateway Commands for TUI
 *
 * Provides /profile and /teams commands that are only visible when connected to Nova Gateway.
 */
...
import type { Organization } from "@novacode/nova-gateway"
...
/**
 * Register all Nova Gateway commands
...
export function registerKiloCommands(useSDK: () => UseSDK) {
  ...
  // Only show Kilo commands when connected to Nova Gateway
  ...
  command.register(() => [
    // /profile command
    {
      value: "kilo.profile",
      title: "Profile",
      description: "View your Nova Gateway profile",
      category: "Kilo",
        ...
        try {
          ...
          if (response.error || !response.data) {
            dialog.replace(() => (
              <DialogAlert
                title="Error"
                message="Failed to fetch profile. Please ensure you're authenticated with Nova Gateway."
              />
            ))
            return
          }
    ...
    // /teams command
    {
      value: "kilo.teams",
      title: "Teams",
      description: "Switch between Nova Gateway teams",
      category: "Kilo",
        ...
        try {
          ...
          if (response.error || !response.data) {
            dialog.replace(() => (
              <DialogAlert
                title="Error"
                message="Failed to fetch teams. Please ensure you're authenticated with Nova Gateway."
              />
            ))
            return
          }
