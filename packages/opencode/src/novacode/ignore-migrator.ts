...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "novacode.ignore-migrator" })

  const KILOCODEIGNORE_FILE = ".novacodeignore"
  const GLOBAL_KILOCODEIGNORE = path.join(os.homedir(), ".novacode", KILOCODEIGNORE_FILE)
  ...
  /**
   * Parse .novacodeignore content into patterns.
  ...
  /**
   * Load patterns from a .novacodeignore file
   */
  ...
  /**
   * Migrate .novacodeignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .novacodeignore (lower priority)
    if (!options.skipGlobalPaths) {
      ...
      if (globalPatterns.length > 0) {
        log.debug("loaded global .novacodeignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .novacodeignore (higher priority - added last)
    ...
    if (projectPatterns.length > 0) {
      log.debug("loaded project .novacodeignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .novacodeignore and return permission config.
  ...
  export async function loadIgnoreConfig(projectDir: string, skipGlobalPaths?: boolean): Promise<Config.Permission> {
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .novacodeignore patterns", {
      ...
      log.warn("failed to load .novacodeignore", { error: err })
      return {}
    }
  }
}
