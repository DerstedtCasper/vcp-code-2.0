...
import { IngestQueue } from "@/nova-sessions/ingest-queue"
import { clearInFlightCache, withInFlightCache } from "@/nova-sessions/inflight-cache"
import type * as SDK from "@novacode/sdk/v2"
import z from "zod"
import { KILO_API_BASE } from "@novacode/nova-gateway"
import { Instance } from "@/project/instance"
import { Vcs } from "@/project/vcs"
import simpleGit from "simple-git"

export namespace KiloSessions {
  const log = Log.create({ service: "nova-sessions" })

  const Uuid = z.uuid()
  type Uuid = z.infer<typeof Uuid>

  const tokenValidKeyTemplate = "nova-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "nova-sessions:token"
  const orgKey = "nova-sessions:org"
  const clientKey = "nova-sessions:client"
  const gitUrlKeyPrefix = "nova-sessions:git-url:"
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await novacodeToken()
