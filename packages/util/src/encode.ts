export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // novacode_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // novacode_change - encode as UTF-8 bytes
