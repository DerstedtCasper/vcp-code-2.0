---
title: "Custom Rules"
description: "Define custom rules for Nova Code behavior"
---

# Custom Rules

Custom rules provide a powerful way to define project-specific and global behaviors and constraints for the Nova Code AI agent. With custom rules, you can ensure consistent formatting, restrict access to sensitive files, enforce coding standards, and customize the AI's behavior for your specific project needs or across all projects.
...
Nova Code supports two types of custom rules:
...
Custom rules are primarily loaded from the **`.novacode/rules/` directory**. This is the recommended approach for organizing your project-specific rules. Each rule is typically placed in its own Markdown file with a descriptive name:
...
Nova Code provides a built-in interface for managing your custom rules without manually editing files in the `.novacode/rules/` directories. To access the UI, click on the <Codicon name="law" /> icon in the **bottom right corner** of the Nova Code window.
...
1. **Global rules** from `~/.novacode/rules/` directory
2. **Project rules** from `.novacode/rules/` directory
3. **Legacy fallback files** (for backward compatibility):
   - `.roorules`
   - `.clinerules`
   - `.novacoderules` (deprecated)
...
{% callout type="note" %}
We strongly recommend keeping your rules in the `.novacode/rules/` folder as it provides better organization and is the preferred approach for future versions. The folder-based structure allows for more granular rule organization and clearer separation of concerns. The legacy file-based approach is maintained for backward compatibility but may be subject to change in future releases.
{% /callout %}
...
1. First, it checks for `.novacode/rules-${mode}/` directory
2. If that doesn't exist or is empty, it falls back to `.novacoderules-${mode}` file (deprecated)
...
{% image src="/docs/img/custom-rules/rules-ui.png" alt="Rules tab in Nova Code" width="400" /%}

The easiest way to create and manage rules is through the built-in UI:

1. Access the rules management interface from the Nova Code panel
...
1. Create the `.novacode/rules/` directory if it doesn't already exist
...
Rules will be automatically applied to all future Nova Code interactions. Any new changes will be applied immediately.
...
{% image src="/docs/img/custom-rules/custom-rules.png" alt="Nova Code ignores request to read sensitive file" width="600" /%}
...
When working in team environments, placing `.novacode/rules/codestyle.md` files under version control allows you to standardize Kilo's behavior across your entire development team. This ensures consistent code style, documentation practices, and development workflows for everyone on the project.
{% /callout %}
...
1. **Check rule locations**: Ensure that your rules are located in supported locations:
   - Global rules: `~/.novacode/rules/` directory
   - Project rules: `.novacode/rules/` directory
   - Legacy files: `.novacoderules`, `.roorules`, or `.clinerules`
