---
title: "Skills"
description: "Extend Nova Code capabilities with skills"
---

# Skills

Nova Code implements [Agent Skills](https://agentskills.io/), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when Nova Code initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.novacode` directory within your Home directory.

- Mac and Linux: `~/.novacode/skills/`
- Windows: `\Users\<yourUser>\.novacode\`
...
Located in `.novacode/skills/` within your project:
...
```bash
# For Code mode only
mkdir -p ~/.novacode/skills-code/typescript-patterns

# For Architect mode only
mkdir -p ~/.novacode/skills-architect/microservices
```
...
When multiple skills share the same name, Nova Code uses these priority rules:
...
Skills are discovered when Nova Code initializes:

- When VSCode starts
- When you reload the VSCode window (`Cmd+Shift+P` → "Developer: Reload Window")

Skills directories are monitored for changes to `SKILL.md` files. However, the most reliable way to pick up new skills is to reload VS or the Nova Code extension.
...
In Nova Code, the `name` field **must match** the parent directory name:
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.novacode/skills/api-design
   ```
...
1. **Check the Output panel**: Open `View` → `Output` → Select "Nova Code" from dropdown. Look for skill-related errors.
