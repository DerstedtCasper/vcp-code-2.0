...
description: "Use your own API keys with Nova Gateway while retaining platform features"
---

# Bring Your Own Key (BYOK)

Bring Your Own Key (BYOK) lets you use your own API keys when using the Nova Gateway, while retaining Kilo platform features like Code Reviews and Cloud Agents.

A user or organization may want to use BYOK to:

- Utilize new models quickly, Nova Gateway supports most new models in minutes
...
Nova Gateway currently supports BYOK keys for these providers:
...
- When you use the **Nova Gateway** provider, Kilo checks if there's a BYOK key for the selected model's provider.
...
- BYOK works with the Nova Gateway provider. Users should ensure that is set as the active [provider](/docs/ai-providers).
