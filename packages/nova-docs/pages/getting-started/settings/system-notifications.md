...
description: "Configure native OS notifications for Nova Code"
---
...
Nova Code uses system notifications to inform you about:
...
Nova Code's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. Nova Code uses macOS's built-in command to display notifications.
...
**How it works:** Nova Code first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
