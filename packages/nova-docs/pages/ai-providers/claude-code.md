...
In January 2026, Anthropic decided to restrict Claude Code CLI to official Claude Code clients. Claude Code credentials cannot be used in Nova Code or other third-party harnesses.

For continued use of Anthropic models in Nova Code, please use the [Anthropic API provider](/docs/ai-providers/anthropic) with an API key instead.
{% /callout %}

# Using Claude Code With Nova Code

Claude Code is Anthropic's official CLI that provides direct access to Claude models from your terminal. Using Claude Code with Nova Code lets you leverage your existing CLI setup without needing separate API keys.
...
If this environment variable is set on your system, the `claude` tool may use it for authentication instead of the interactive `/login` method. When Nova Code executes the tool, it will accurately reflect that an API key is being used, as this is the underlying behavior of the `claude` CLI itself.
{% /callout %}
...
1. **Open Nova Code Settings:** Click the gear icon ({% codicon name="gear" /%}) in the Nova Code panel.
...
- **Enhance Prompt Feature:** Full compatibility with Nova Code's Enhance Prompt feature, allowing you to automatically improve and refine your prompts before sending them to Claude.
