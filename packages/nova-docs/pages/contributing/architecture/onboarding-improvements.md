...
Users aren't guided through Nova Code's modes or key features. The existing tab-based tutorial is easily dismissed, causing users to miss critical functionality.
...
├── Content: "Welcome to NovaCode! Let's take a quick tour."
...
The "Set API Key" button is at the bottom of settings, making Nova Code setup hard to discover and complete.

**Improvements:**

- Move "Set API Key" button next to API key input field
- Rearrange layout for better flow
- Make Nova Code provider setup prominent
...
- Integration with Nova Code teams for company/repo-personalized onboarding
