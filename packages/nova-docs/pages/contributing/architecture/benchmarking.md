---
title: "Benchmarking"
description: "Design for benchmarking Nova Code against models and other agents"
---
...
This document proposes a benchmarking system for Nova Code with two primary goals:

1. **Compare models against one another** using the same agent -- measuring task completion, token cost, and total time
2. **Compare agents against one another** using the same model -- e.g., Nova Code vs Claude Code, or Nova Code v1.0 vs v1.1
...
The key engineering deliverable is a **Nova Code Harbor adapter** that runs Kilo CLI autonomously in containerized environments and emits ATIF-compliant trajectories.
...
As Nova Code evolves, we need systematic answers to questions like:
...
- How does Nova Code compare to Claude Code, Codex, or other agents on the same tasks?
...
1. Run Nova Code against standardized benchmark datasets in a reproducible, containerized environment
...
Harbor is the standard evaluation framework used by many frontier labs. Rather than building our own harness, we write a Nova Code adapter and plug into the existing ecosystem.
...
This makes it easy to create task sets that target specific Nova Code capabilities -- for example, a set of refactoring tasks, or a set of multi-file debugging scenarios. Custom sets can be published to the Harbor registry or kept private.

See the [Harbor task tutorial](https://www.tbench.ai/docs/task-tutorial) for a step-by-step guide.

## Deliverables

### 1. Nova Code Harbor Adapter
...
Configure the Opik-Harbor integration for Nova Code benchmark runs:

- Set up `opik harbor run` with the Nova Code adapter
...
Run the same Nova Code agent against Terminal-Bench with different models:
