...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Nova Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Nova Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Nova Code
...
- Single-client scenarios (one Nova Code instance per server)
...
1. The client (Nova Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Nova Code
...
STDIO servers run locally on the same machine as Nova Code, which has several important implications:
...
- **Execution**: Starts and stops with Nova Code (child process lifecycle)
...
- Start when needed by Nova Code
- Not require network configuration
- Need to be installed alongside Nova Code or via a package manager
...
## Configuring Transports in Nova Code

For detailed information on configuring STDIO and SSE transports in Nova Code, including example configurations, see the [Understanding Transport Types](using-in-kilo-code#understanding-transport-types) section in the Using MCP in Nova Code guide.
