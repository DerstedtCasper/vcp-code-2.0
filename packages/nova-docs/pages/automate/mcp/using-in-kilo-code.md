---
title: "Using MCP in Nova Code"
description: "How to use MCP servers in Nova Code"
---

# Using MCP in Nova Code

Model Context Protocol (MCP) extends Nova Code's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Nova Code.

{% youtube url="https://youtu.be/6O9RQoQRX8A" caption="Demostrating MCP installation in Nova Code" /%}
...
2.  **Project-level Configuration**: Defined in a `.novacode/mcp.json` file within your project's root directory. This allows you to set up project-specific servers and share configurations with your team by committing the file to version control. Nova Code automatically detects and loads this file if it exists.
...
You can edit both global and project-level MCP configuration files directly from the Nova Code settings.

1. Click the {% codicon name="gear" /%} icon in the top navigation of the Nova Code pane to open `Settings`.
2. Click the `Agent Behaviour` tab on the left side
3. Select the `MCP Servers` sub-tab
4. Click the appropriate button:
   - **`Edit Global MCP`**: Opens the global `mcp_settings.json` file.
   - **`Edit Project MCP`**: Opens the project-specific `.novacode/mcp.json` file. If this file doesn't exist, Nova Code will create it for you.
...
_Example of MCP Server config in Nova Code (STDIO Transport)_
...
When enabled, Nova Code will automatically approve this specific tool without prompting. Note that the global "Use MCP servers" setting takes precedence - if it's disabled, no MCP tools will be auto-approved.

## Finding and Installing MCP Servers

Nova Code does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Nova Code:** You can ask Nova Code to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Nova Code with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Nova Code will automatically detect available tools and resources. To use them:

1. Type your request in the Nova Code chat interface
2. Nova Code will identify when an MCP tool can help with your task
...
- **Permission Errors:** Ensure proper API keys and credentials are configured in your `mcp_settings.json` (for global settings) or `.novacode/mcp.json` (for project settings).
