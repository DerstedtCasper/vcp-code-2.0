...
The `list_files` tool displays the files and directories within a specified location. It helps Nova Code understand your project structure and navigate your codebase effectively.
...
- When Nova Code needs to understand your project structure
- When Nova Code explores what files are available before reading specific ones
- When Nova Code maps a codebase to better understand its organization
- Before using more targeted tools like `read_file` or `search_files`
- When Nova Code needs to check for specific file types (like configuration files) across a project
...
- Marks files ignored by `.novacodeignore` with a lock symbol (🔒) when `showNovaCodeIgnoredFiles` is enabled
...
5. **Result Filtering**:
   ...
   - Respects `.gitignore` rules when in recursive mode
   - Handles `.novacodeignore` patterns, either hiding files or marking them with a lock symbol
6. **Formatting**:
   - Marks directories with a trailing slash (`/`)
   - Sorts results to show directories before their contents for logical hierarchy
   - Marks ignored files with a lock symbol (🔒) when `showNovaCodeIgnored` is enabled
...
- Files ignored by `.novacodeignore` are marked with a lock symbol (🔒) when `showNovaCodeIgnored` is enabled
...
When `.novacodeignore` files are used and `showNovaCodeIgnored` is enabled:
...
- When starting a new task, Nova Code may list the project files to understand its structure before diving into specific code.
- When asked to find specific types of files (like all JavaScript files), Nova Code first lists directories to know where to look.
- When providing recommendations for code organization, Nova Code examines the current project structure first.
- When setting up a new feature, Nova Code lists related directories to understand the project conventions.
