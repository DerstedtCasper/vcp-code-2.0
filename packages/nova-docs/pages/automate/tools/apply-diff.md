...
- When Nova Code needs to make precise changes to existing code without rewriting entire files.
...
- Validates file access against `.novacodeignore` rules.
...
2.  **NovaCodeIgnore Check**: Validates if the target file path is allowed by `.novacodeignore` rules.
...
Nova Code uses this strategy for applying diffs:
