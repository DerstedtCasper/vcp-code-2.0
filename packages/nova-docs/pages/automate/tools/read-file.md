...
The `read_file` tool examines the contents of files in a project. It allows Nova Code to understand code, configuration files, and documentation to provide better assistance.
...
- When Nova Code needs to understand existing code structure
- When Nova Code needs to analyze configuration files
- When Nova Code needs to extract information from text files
- When Nova Code needs to see code before suggesting changes
...
- When asked to explain or improve code, Nova Code first reads the relevant files to understand the current implementation.
- When troubleshooting configuration issues, Nova Code reads config files to identify potential problems.
- When working with documentation, Nova Code reads existing docs to understand the current content before suggesting improvements.
...
If the file is excluded by rules in a `.novacodeignore` file:
...
```
Error: Access denied to file '.env' due to .novacodeignore rules.
```
