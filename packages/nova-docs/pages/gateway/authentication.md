...
API keys are JWT tokens tied to your Kilo account. See [how to get your API key](/docs/getting-started/setup-authentication#nova-gateway-api-key) for step-by-step instructions.
...
```
X-NovaCode-OrganizationId: your_org_id
```
...
1. Add your provider API key in the Kilo dashboard or through your Nova Code extension settings
...
```
Client → Nova Gateway → Vercel AI Gateway (with your key) → Provider
```
...
| `X-NovaCode-OrganizationId` | No                      | Organization context for org-scoped requests |
| `X-NovaCode-TaskId`         | No                      | Task identifier for prompt cache keying      |
| `X-NovaCode-Version`        | No                      | Client version string                        |
| `x-novacode-mode`           | No                      | Mode hint for `nova/auto` model routing      |
