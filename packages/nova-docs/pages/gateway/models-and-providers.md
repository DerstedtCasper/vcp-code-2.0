...
The `nova/auto` virtual model automatically selects the best model based on the task type. The selection is controlled by the `x-novacode-mode` request header:
...
curl -X POST "https://api.kilo.ai/api/gateway/chat/completions" \
  ...
  -H "x-novacode-mode: plan" \
  -H "Content-Type: application/json" \
  -d '{"model": "nova/auto", "messages": [{"role": "user", "content": "Design a database schema"}]}'
```
