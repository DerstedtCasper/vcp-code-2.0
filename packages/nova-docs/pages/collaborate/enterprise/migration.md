---
title: "Migration"
description: "Migrate your team to Nova Code Enterprise"
---
...
2. **Install Nova Code extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on Nova Code features and workflows
...
2. **Install Nova Code** extension for team members
3. **Start with simple tasks** in Nova Code
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use Nova Code** for new projects
...
- [ ] Install Nova Code extension across team
...
Nova Code uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.novacode/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.novacode/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Nova Code (no conversion needed)
- **Global rules**: Recreate in `~/.novacode/rules/*.md` directory

Nova Code also supports mode-specific rules (`.novacode/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- Nova Code extension installation and setup
