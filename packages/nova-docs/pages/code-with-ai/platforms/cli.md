---
title: "Kilo CLI"
description: "Using Nova Code from the command line"
---
...
The Nova Code CLI uses the same underlying technology that powers the IDE extensions, so you can expect the same workflow to handle agentic coding tasks from start to finish.

**Source code & issues (Kilo CLI 1.0):** [DerstedtCasper/vcp-code-2.0](https://github.com/DerstedtCasper/vcp-code-2.0) · [Report an issue](https://github.com/DerstedtCasper/vcp-code-2.0/issues)
...
`npm update -g @novacode/cli`

## What you can do with Nova Code CLI
...
#### Nova Gateway Commands (when connected)

| Command    | Aliases                  | Description                       |
| ---------- | ------------------------ | --------------------------------- |
| `/profile` | `/me`, `/whoami`         | View your Nova Gateway profile    |
| `/teams`   | `/team`, `/org`, `/orgs` | Switch between Nova Gateway teams |
...
Nova Code uses the permission config to decide whether a given action should run automatically, prompt you, or be blocked.
...
The Kilo CLI is a fork of [OpenCode](https://opencode.ai) and supports the same configuration options. The CLI you install with `npm install -g @novacode/cli` (Kilo CLI 1.0) is built from [DerstedtCasper/vcp-code-2.0](https://github.com/DerstedtCasper/vcp-code-2.0). For comprehensive configuration documentation, see the [OpenCode Config documentation](https://opencode.ai/docs/config).
...
Interactive mode is the default mode when running Nova Code without the `--auto` flag, designed to work interactively with a user through the console.

In interactive mode Nova Code will request approval for operations which have not been auto-approved, allowing the user to review and approve operations before they are executed, and optionally add them to the auto-approval list.
...
Autonomous mode allows Nova Code to run in automated environments like CI/CD pipelines without requiring user interaction.
...
- For `novacode` provider: `KILOCODE_<FIELD_NAME>` (e.g., `KILOCODE_MODEL` → `novacodeModel`)
