...
# Migrating from Cline to VCP

A practical guide for developers switching from Cline to VCP.

## Why Switch?

**Speed.** The VCP Platform is designed to reduce friction across the entire development workflow. Beyond feature parity with Cline, VCP includes Code Reviews, App Builder, Cloud Agents, and one-click Deploy.

**Multi-platform.** Work in VS Code, JetBrains IDEs, CLI, or the web. Your Sessions sync across all of them automatically.

**Specialized modes.** Instead of one agent doing everything, VCP has five specialized modes optimized for different parts of your workflow.

**500+ models.** More providers, more options, better pricing flexibility. Switch models mid-conversation if you want.

---

## Understanding VCP's Mode System

Cline uses a single agent with a Plan/Act toggle. VCP takes a different approach: five specialized modes, each optimized for a specific part of development. You select modes from a dropdown in the interface in the VCP Extension, or with slash commands in the CLI.
...
**VCP:** Select **Ask** from the dropdown
...
**Why this matters:** In Cline, you might accidentally make changes while exploring. In VCP, Ask and Architect modes can't write files, so you're safe to explore without worry. VCP's **Orchestrator** mode lets you intelligently generate and execute subtasks that are optimized for each mode.
...
2. Search "VCP Code"
3. Click Install
4. Find the VCP icon in your sidebar
...
4. Find VCP icon in right sidebar
...
Visit [app.VCP.ai](https://github.com/DerstedtCasper/vcp-code-2.0/) and sign in. This gives you access to:

- **Cloud Agents:** Run VCP without a local machine
- **App Builder:** Build and preview apps directly in your browser
- **VCP Deploy:** One-click deployments
...
1. Click "Try VCP Code for Free" in the VCP panel
2. Sign in with OAuth at VCP.ai
3. You'll be redirected back to your IDE

### Configure your provider

**Option 1: Use VCP Gateway (easiest)**

1. Open settings (⚙️ icon)
2. Select "VCP Gateway" as provider
...
## Beyond the IDE: VCP's Platform Features

One of the biggest differences from Cline is that VCP isn't just an IDE extension. It's a platform with multiple interfaces that can all share your Sessions and context.

### Cloud Agents

Run VCP from [app.VCP.ai/cloud](https://github.com/DerstedtCasper/vcp-code-2.0/cloud) without needing your local machine. Great for:
...
Build live apps and sites directly from the web with a real-time preview. Similar to Lovable, but integrated with your VCP Sessions. Deploy with one click when you're ready.

### VCP Deploy

One-click deployments from directly within VCP. Go from code to production without leaving your workflow.
...
Semantic search across your repositories using cloud-hosted embeddings. VCP indexes your codebase to deliver more relevant, context-aware responses.
...
| Cline Feature      | VCP Equivalent                    | Notes                                                                           |
| ------------------ | ---------------------------------- | ------------------------------------------------------------------------------- |
...
- **One-click Deploy:** Ship directly from VCP
- **Code Reviews:** AI-powered PR analysis
...
**Q: Can I use both Cline and VCP side-by-side?** Yes. They're separate extensions.
...
1. Install VCP in your primary IDE
...
3. Try Cloud Agents at [app.VCP.ai](https://github.com/DerstedtCasper/vcp-code-2.0/)
4. Install the VCP CLI with `npm install -g @VCPcode/cli`
