...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between VCP Code and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (VCP Code) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as VCP Code
...
- Single-client scenarios (one VCP Code instance per server)
...
1. The client (VCP Code) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from VCP Code
...
STDIO servers run locally on the same machine as VCP Code, which has several important implications:
...
- **Execution**: Starts and stops with VCP Code (child process lifecycle)
...
- Start when needed by VCP Code
- Not require network configuration
- Need to be installed alongside VCP Code or via a package manager
...
## Configuring Transports in VCP Code

For detailed information on configuring STDIO and SSE transports in VCP Code, including example configurations, see the [Understanding Transport Types](using-in-VCP-code#understanding-transport-types) section in the Using MCP in VCP Code guide.
