...
The `read_file` tool examines the contents of files in a project. It allows VCP Code to understand code, configuration files, and documentation to provide better assistance.
...
- When VCP Code needs to understand existing code structure
- When VCP Code needs to analyze configuration files
- When VCP Code needs to extract information from text files
- When VCP Code needs to see code before suggesting changes
...
- When asked to explain or improve code, VCP Code first reads the relevant files to understand the current implementation.
- When troubleshooting configuration issues, VCP Code reads config files to identify potential problems.
- When working with documentation, VCP Code reads existing docs to understand the current content before suggesting improvements.
...
If the file is excluded by rules in a `.VCPcodeignore` file:
...
```
Error: Access denied to file '.env' due to .VCPcodeignore rules.
```
