...
description: "Integrate with the VCP Gateway using the Vercel AI SDK, OpenAI SDK, Python, cURL, or any OpenAI-compatible client."
---

# SDKs & Frameworks

The VCP Gateway is OpenAI-compatible, meaning any SDK or framework that works with the OpenAI API can work with the VCP Gateway by changing the base URL.
...
The official OpenAI SDKs work with the VCP Gateway by setting the base URL.
...
```bash
curl -N -X POST "https://api.VCP.ai/api/gateway/chat/completions" \
  -H "Authorization: Bearer $VCP_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "anthropic/claude-sonnet-4.5",
...
func main() {
    ...
    req, _ := http.NewRequest("POST",
        "https://api.VCP.ai/api/gateway/chat/completions",
        bytes.NewBuffer(jsonBody))

    req.Header.Set("Authorization", "Bearer "+os.Getenv("VCP_API_KEY"))
...
uri = URI('https://api.VCP.ai/api/gateway/chat/completions')
http = Net::HTTP.new(uri.host, uri.port)
http.use_ssl = true

request = Net::HTTP::Post.new(uri)
request['Authorization'] = "Bearer #{ENV['VCP_API_KEY']}"
...
The VCP Gateway works with any framework that supports OpenAI-compatible APIs:

| Framework                                                             | Integration                               |
| --------------------------------------------------------------------- | ----------------------------------------- |
| [Vercel AI SDK](https://ai-sdk.dev)                                   | Use `createOpenAI` with VCP base URL     |
...
const model = new ChatOpenAI({
  ...
  openAIApiKey: process.env.VCP_API_KEY,
  configuration: {
    baseURL: "https://api.VCP.ai/api/gateway",
  },
})
