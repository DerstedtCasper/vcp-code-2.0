---
title: "Custom Rules"
description: "Define custom rules for VCP Code behavior"
---

# Custom Rules

Custom rules provide a powerful way to define project-specific and global behaviors and constraints for the VCP Code AI agent. With custom rules, you can ensure consistent formatting, restrict access to sensitive files, enforce coding standards, and customize the AI's behavior for your specific project needs or across all projects.
...
VCP Code supports two types of custom rules:
...
Custom rules are primarily loaded from the **`.VCPcode/rules/` directory**. This is the recommended approach for organizing your project-specific rules. Each rule is typically placed in its own Markdown file with a descriptive name:
...
VCP Code provides a built-in interface for managing your custom rules without manually editing files in the `.VCPcode/rules/` directories. To access the UI, click on the <Codicon name="law" /> icon in the **bottom right corner** of the VCP Code window.
...
1. **Global rules** from `~/.VCPcode/rules/` directory
2. **Project rules** from `.VCPcode/rules/` directory
3. **Legacy fallback files** (for backward compatibility):
   - `.roorules`
   - `.clinerules`
   - `.VCPcoderules` (deprecated)
...
{% callout type="note" %}
We strongly recommend keeping your rules in the `.VCPcode/rules/` folder as it provides better organization and is the preferred approach for future versions. The folder-based structure allows for more granular rule organization and clearer separation of concerns. The legacy file-based approach is maintained for backward compatibility but may be subject to change in future releases.
{% /callout %}
...
1. First, it checks for `.VCPcode/rules-${mode}/` directory
2. If that doesn't exist or is empty, it falls back to `.VCPcoderules-${mode}` file (deprecated)
...
{% image src="/docs/img/custom-rules/rules-ui.png" alt="Rules tab in VCP Code" width="400" /%}

The easiest way to create and manage rules is through the built-in UI:

1. Access the rules management interface from the VCP Code panel
...
1. Create the `.VCPcode/rules/` directory if it doesn't already exist
...
Rules will be automatically applied to all future VCP Code interactions. Any new changes will be applied immediately.
...
{% image src="/docs/img/custom-rules/custom-rules.png" alt="VCP Code ignores request to read sensitive file" width="600" /%}
...
When working in team environments, placing `.VCPcode/rules/codestyle.md` files under version control allows you to standardize VCP's behavior across your entire development team. This ensures consistent code style, documentation practices, and development workflows for everyone on the project.
{% /callout %}
...
1. **Check rule locations**: Ensure that your rules are located in supported locations:
   - Global rules: `~/.VCPcode/rules/` directory
   - Project rules: `.VCPcode/rules/` directory
   - Legacy files: `.VCPcoderules`, `.roorules`, or `.clinerules`
