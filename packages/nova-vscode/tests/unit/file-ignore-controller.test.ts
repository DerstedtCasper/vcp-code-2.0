...
describe("FileIgnoreController", () => {
  describe("when .novacodeignore exists", () => {
    it("applies only .novacodeignore patterns", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".novacodeignore"), "secret/**\n*.snap\n")
      ...
      expect(controller.getInstructions()).toContain(".novacodeignore")
    })

    it("does NOT block .env files unless explicitly listed", async () => {
      const workspace = await createTempWorkspace()
      await fs.writeFile(path.join(workspace, ".novacodeignore"), "dist/\n")
    ...
    it("does NOT apply .gitignore patterns", async () => {
      ...
      // .novacodeignore pattern should apply
      expect(controller.validateAccess(path.join(workspace, "dist", "bundle.js"))).toBe(false)
    })
  })

  describe("when no .novacodeignore exists (fallback)", () => {
    ...
    it("blocks .env files via hardcoded sensitive patterns", async () => {
      const workspace = await createTempWorkspace()
      // No .novacodeignore, no .gitignore
