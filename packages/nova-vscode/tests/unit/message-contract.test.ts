 ...
 * 2. Every WebviewMessage type handled in NovaProvider has a corresponding member in the WebviewMessage union
...
const KILO_PROVIDER_FILE = path.join(ROOT, "src/NovaProvider.ts")
const KILO_PROVIDER_UTILS_FILE = path.join(ROOT, "src/nova-provider-utils.ts")
...
describe("NovaProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in NovaProvider exist in WebviewMessage union", () => {
    ...
    expect(
      unrecognized,
      `NovaProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
