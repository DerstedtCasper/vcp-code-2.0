...
describe("WorktreeStateManager", () => {
  ...
  beforeEach(() => {
    ...
    // Pre-create .novacode dir so fire-and-forget saves don't race on mkdir
    fs.mkdirSync(path.join(root, ".novacode"), { recursive: true })
    logs.length = 0
  ...
  describe("persistence", () => {
    ...
    it("creates .novacode directory if missing", async () => {
      ...
      expect(fs.existsSync(path.join(fresh, ".novacode", "agent-manager.json"))).toBe(true)
    })
  })
  ...
  describe("tab order", () => {
    ...
    it("does not persist empty tab order", async () => {
      ...
      const content = fs.readFileSync(path.join(root, ".novacode", "agent-manager.json"), "utf-8")
  ...
  describe("validate", () => {
    ...
    it("resolves relative paths against root", async () => {
      const relative = ".novacode/worktrees/test-branch"
  ...
  describe("load with corrupt data", () => {
    it("handles malformed JSON gracefully", async () => {
      const file = path.join(root, ".novacode", "agent-manager.json")
