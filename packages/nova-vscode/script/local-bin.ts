...
/**
 * Ensures the VS Code extension has a CLI binary at `packages/nova-vscode/bin/kilo`.
 *
 * Strategy:
 ...
 * 4) Copy the resulting binary into `packages/nova-vscode/bin/kilo` and chmod +x.
 *
 * This script is intended to be run from `packages/nova-vscode` as part of build/package.
 */
...
async function findKiloBinaryInOpencodeDist(): Promise<string | null> {
  ...
  const preferred = join(distDir, `@novacode`, tag, "bin", binName)
  try {
    statSync(preferred)
    return preferred
  } catch {
