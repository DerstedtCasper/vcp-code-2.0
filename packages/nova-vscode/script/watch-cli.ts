 ...
 * then copies it into packages/nova-vscode/bin/kilo.
 *
...
const targetBinPath = join(targetBinDir, "nova")
...
function sourceBinaryPath(): string {
  return join(opencodeDir, "dist", `@novacode/cli-${process.platform}-${process.arch}`, "bin", "nova")
}
