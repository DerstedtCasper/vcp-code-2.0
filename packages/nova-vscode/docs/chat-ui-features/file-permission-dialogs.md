...
Permissions are now rendered through nova-ui's `DataProvider` pattern using `Dialog` + `BasicTool` + `data-component="permission-prompt"`. There is no standalone `BatchFilePermission.tsx` — the permission UI is integrated into the nova-ui message rendering pipeline.
...
- Kilo CLI UI reference: permission prompt actions exist in [`packages/ui/src/components/message-part.tsx`](https://github.com/Kilo-Org/novacode/blob/main/packages/ui/src/components/message-part.tsx:1).
