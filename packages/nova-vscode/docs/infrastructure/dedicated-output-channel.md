...
All extension logging uses `console.log("[Kilo New] ...")` which goes to the Extension Host output channel mixed with all other extensions. A dedicated "Nova Code" output channel would make logs easier to find and filter.

## Requirements

- Create a `vscode.window.createOutputChannel("Nova Code")` during activation
...
const output = vscode.window.createOutputChannel("Nova Code")
