...
The CLI binary check at [`ServerManager.startServer()`](../../src/services/cli-backend/server-manager.ts:52) throws an error string that is caught in [`NovaProvider.initializeConnection()`](../../src/NovaProvider.ts:261) and sent to the webview only.
...
vscode.window.showErrorMessage(`Nova Code: Failed to start CLI server — ${error.message}`, "Retry").then((action) => {
...
- [`src/NovaProvider.ts`](../../src/NovaProvider.ts) — add `vscode.window.showErrorMessage()` calls in error paths
