...
Nova Code is an open source AI coding agent platform. It ships as multiple products that all build on the same backend. This package (`packages/nova-vscode/`) is the **VS Code extension** — one of several clients.

### Products and How They Relate

All products are thin clients over the **CLI** (`packages/opencode/`, published as `@novacode/cli`). The CLI is a fork of upstream [OpenCode](https://github.com/anomalyco/opencode) with Kilo-specific additions (gateway auth, telemetry, migration, code review, branding). It contains the full AI agent runtime, tool execution, session management, provider integrations (500+ models), and an HTTP API server.

Every client spawns or connects to a `kilo serve` process and communicates via HTTP REST + SSE using the auto-generated `@novacode/sdk`.

```
                        @novacode/cli  (packages/opencode/)
                     ┌────────────────────────────────┐
...
| **Kilo VS Code Extension** | **`packages/nova-vscode/`**            | VS Code extension with sidebar chat + Agent Manager | Bundles CLI binary, spawns `kilo serve --port 0` as child process |
...
| Package                    | Name                       | Role                                                                                                                         |
| -------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `packages/nova-vscode/`    | `kilo-code`                | **This package.** VS Code extension.                                                                                         |
| `packages/nova-gateway/`   | `@novacode/nova-gateway`   | Auth (device flow), AI provider routing (OpenRouter), Kilo API integration (profile, balance, teams)                         |
| `packages/nova-ui/`        | `@novacode/nova-ui`        | SolidJS component library (40+ components, built on `@kobalte/core`). Shared by this extension's webview and `packages/app/` |
| `packages/nova-telemetry/` | `@novacode/nova-telemetry` | PostHog analytics + OpenTelemetry tracing for the CLI                                                                        |
| `packages/nova-i18n/`      | `@novacode/nova-i18n`      | Translation strings (16 languages)                                                                                           |
| `packages/nova-docs/`      | `@novacode/nova-docs`      | Documentation site (Next.js + Markdoc)                                                                                       |

### Upstream OpenCode Packages (not Kilo-specific)

| Package              | Name                   | Role                                                                                     |
| -------------------- | ---------------------- | ---------------------------------------------------------------------------------------- |
| `packages/opencode/` | `@novacode/cli`        | Core CLI — forked from upstream OpenCode. AI agents, tools, sessions, server.            |
...
│                          │                │   AI agent runtime   │
│ NovaProvider (sidebar)   │                └──────────────────────┘
│ NovaProvider (agent mgr) │
│ NovaProvider (open tabs) │
└──────────────────────────┘
```
...
- Multiple **`NovaProvider`** instances (sidebar, Agent Manager, "open in tab" panels) subscribe to the shared connection. SSE events are filtered per-webview via a `trackedSessionIds` Set.
...
- CSP requires nonce for scripts and `font-src` for bundled fonts — see [`NovaProvider.ts`](src/NovaProvider.ts:777)
- HTML root has `data-theme="nova-vscode"` to activate nova-ui's VS Code theme bridge
...
3. **NovaProvider** (`src/NovaProvider.ts`): Add a `fetchAndSend*()` method using the cached message pattern, and handle the corresponding `request*` message from the webview in `handleWebviewMessage()`
...
- **Cached messages** (e.g. `cachedProvidersMessage`, `cachedAgentsMessage` in NovaProvider): Ensures webview refreshes get data immediately without waiting for a new HTTP round-trip
...
| State         | No dedicated state file    | `.novacode/agent-manager.json`                          |
| Terminals     | None                       | Dedicated VS Code terminal per session                  |
| Setup scripts | None                       | Configurable `.novacode/setup-script` runs per worktree |
...
New webview features must use **`@novacode/nova-ui`** components instead of raw HTML elements with inline styles. This is a Solid.js component library built on `@kobalte/core`.

- Import via deep subpaths: `import { Button } from "@novacode/nova-ui/button"`
...
- Global styles imported via `import "@novacode/nova-ui/styles"` in [`index.tsx`](webview-ui/src/index.tsx:2)
- [`chat.css`](webview-ui/src/styles/chat.css) is being progressively migrated — when replacing a component with nova-ui, remove the corresponding CSS rules from it
- New CSS for components not yet in nova-ui goes into `chat.css` grouped by comment-delimited sections (`/* Component Name */`). Once a nova-ui equivalent exists, remove the section.
- **Check the desktop app first**: [`packages/app/src/`](../../packages/app/src/) is the reference implementation for how nova-ui components are composed together. Always check how the app uses a component before implementing it in the webview — don't just look at the component API in isolation.
- **`data-component` and `data-slot` attributes carry CSS styling** — nova-ui uses `[data-component]` and `[data-slot]` attribute selectors, not class names. When the app uses e.g. `data-component="permission-prompt"` and `data-slot="permission-actions"`, these get nova-ui styling for free.
- **Prefer nova-ui styles**: Always reuse existing nova-ui CSS variables, tokens, and component styles instead of writing custom CSS. If a style doesn't exist in nova-ui yet, add it there and reuse it rather than inlining or duplicating styles in the webview.
- **Icons**: nova-ui has 75+ custom SVG icons in [`packages/ui/src/components/icon.tsx`](../../packages/ui/src/components/icon.tsx). To list all available icon names: `node -e "const c=require('fs').readFileSync('../../packages/ui/src/components/icon.tsx','utf8');[...c.matchAll(/^\\s{2}[\"']?([\\w-]+)[\"']?:\\s*\x60/gm)].map(m=>m[1]).sort().forEach(n=>console.log(n))"`. Icon names use both hyphenated (`arrow-left`) and bare-word (`brain`, `console`, `providers`) keys.
...
This package is entirely Kilo-specific — `novacode_change` markers are NOT needed in any files under `packages/nova-vscode/`. The markers are only necessary when modifying shared upstream opencode files.
