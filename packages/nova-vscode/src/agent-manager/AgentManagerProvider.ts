 ...
 * sessions are stored in `.novacode/agent-manager.json`. The UI shows two
...
export class AgentManagerProvider implements vscode.Disposable {
  ...
  private provider: NovaProvider | undefined
  ...
  public openPanel(): void {
    ...
    this.panel.iconPath = {
      light: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "nova-light.svg"),
      dark: vscode.Uri.joinPath(this.extensionUri, "assets", "icons", "nova-dark.svg"),
    }

    this.panel.webview.html = this.getHtml(this.panel.webview)

    this.provider = new NovaProvider(this.extensionUri, this.connectionService)
  ...
  private async initializeState(): Promise<void> {
    ...
    // Register all worktree sessions with NovaProvider
  ...
  private async onCreateMultiVersion(msg: Record<string, unknown>): Promise<null> {
    ...
    // Phase 2: Send the initial prompt to all sessions via the NovaProvider's
    ...
    // is sent. We route each message through the webview→NovaProvider pipeline.
