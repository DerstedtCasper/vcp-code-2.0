...
const env = {
  worktreePath: "/repos/project/.novacode/worktrees/wt-1",
  repoPath: "/repos/project",
}

const script = "/repos/project/.novacode/setup-script"
...
describe("buildSetupCommand", () => {
  it("builds unix command with inline env vars and sh", () => {
    ...
    expect(result).toBe(
      `WORKTREE_PATH="/repos/project/.novacode/worktrees/wt-1" REPO_PATH="/repos/project" sh "/repos/project/.novacode/setup-script"`,
    )
  })
  ...
  it("builds windows command with set and call", () => {
    ...
    expect(result).toBe(
      `set "WORKTREE_PATH=/repos/project/.novacode/worktrees/wt-1" && set "REPO_PATH=/repos/project" && call "/repos/project/.novacode/setup-script"`,
    )
  })
  ...
  it("handles paths with spaces", () => {
    const spaced = {
      worktreePath: "/Users/dev/my project/.novacode/worktrees/wt-1",
      repoPath: "/Users/dev/my project",
    }
    const spacedScript = "/Users/dev/my project/.novacode/setup-script"

    const unix = buildSetupCommand(spacedScript, spaced, "darwin")
    expect(unix).toContain(`sh "/Users/dev/my project/.novacode/setup-script"`)

    const win = buildSetupCommand(spacedScript, spaced, "win32")
    expect(win).toContain(`call "/Users/dev/my project/.novacode/setup-script"`)
  })
