...
const KILOCODEIGNORE = ".novacodeignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .novacodeignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .novacodeignore first — if it exists, use only that.
    ...
    const novacodeignorePath = path.join(this.workspacePath, KILOCODEIGNORE)
    if (fs.existsSync(novacodeignorePath)) {
      const novacodeignoreContent = fs.readFileSync(novacodeignorePath, "utf-8")
      if (novacodeignoreContent.trim()) {
        this.ignoreInstance.add(novacodeignoreContent)
        this.ignoreInstance.add(KILOCODEIGNORE)
        this.loadedContents.push({ file: KILOCODEIGNORE, content: novacodeignoreContent })
        return
      }
    }
