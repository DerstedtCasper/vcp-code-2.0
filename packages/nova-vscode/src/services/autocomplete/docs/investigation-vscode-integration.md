...
| `novacode.autocomplete.inline-completion.accepted`         | `src/services/autocomplete/classic-auto-complete/AutocompleteInlineCompletionProvider.ts:313` | Telemetry callback when suggestion accepted |
...
| ---------------------------------------------------------- | -------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `kilo-code.autocomplete.cancelSuggestions`                 | `Escape` | same    | `editorTextFocus && !editorTabMovesFocus && !inSnippetMode && novacode.autocomplete.hasSuggestions`                                                         |
| `kilo-code.autocomplete.generateSuggestions`               | `Ctrl+L` | `Cmd+L` | `editorTextFocus && !editorTabMovesFocus && !inSnippetMode && novacode.autocomplete.enableSmartInlineTaskKeybinding && !github.copilot.completions.enabled` |
| `kilo-code.autocomplete.showIncompatibilityExtensionPopup` | `Ctrl+L` | `Cmd+L` | `editorTextFocus && !editorTabMovesFocus && !inSnippetMode && novacode.autocomplete.enableSmartInlineTaskKeybinding && github.copilot.completions.enabled`  |

**Note**: The `Escape` keybinding references `novacode.autocomplete.hasSuggestions` which is
...
2. Sets context key `novacode.autocomplete.enableSmartInlineTaskKeybinding` via `setContext`
...
| ------------------------------------------------------- | ----------------------------------------------------------------- | -------------------------------------------------------- |
| `novacode.autocomplete.enableSmartInlineTaskKeybinding` | `AutocompleteServiceManager.updateGlobalContext()` (line 286-290) | Controls whether `Cmd+L` / `Ctrl+L` keybinding is active |
| `novacode.autocomplete.hasSuggestions`                  | ❌ Never set                                                      | Would control `Escape` keybinding — currently dead code  |
...
### 5.2 Schema (`packages/types/src/novacode/novacode.ts:9-19`)
...
- Uses `$(nova-logo)` codicon (custom icon font)
...
### 8.2 Runtime i18n keys (src/i18n/locales/en/novacode.json)

Namespace: `novacode:autocomplete.*`

```jsonc
{
  "autocomplete": {
    "statusBar": {
      "enabled": "$(nova-logo) Autocomplete",
      "snoozed": "snoozed",
      "warning": "$(warning) Autocomplete",
      "tooltip": {
        "basic": "Nova Code Autocomplete",
        "disabled": "Nova Code Autocomplete (disabled)",
        "noCredits": "...",
        "noUsableProvider": "...",
    ...
    "toggleMessage": "Nova Code Autocomplete {{status}}",
    "progress": {
      "title": "Nova Code",
    ...
    "input": {
      "title": "Nova Code: Quick Task",
      "placeholder": "e.g., 'refactor this function to be more efficient'",
    },
    "commands": {
      "generateSuggestions": "Nova Code: Generate Suggested Edits",
    ...
    "codeAction": {
      "title": "Nova Code: Suggested Edits",
    },
    "chatParticipant": {
      "fullName": "Nova Code Agent",
      "name": "Agent",
      "description": "I can help you with quick tasks and suggested edits.",
    },
    "incompatibilityExtensionPopup": {
      "message": "The Nova Code Autocomplete is being blocked by a conflict with GitHub Copilot. To fix this, you must disable Copilot's inline suggestions.",
...
Translated in all locale files under `src/i18n/locales/{locale}/novacode.json`.
...
5. **Custom icon font** — `$(nova-logo)` codicon from `assets/icons/kilo-icon-font.woff2`
...
4. **Context keys**: `novacode.autocomplete.enableSmartInlineTaskKeybinding` (via setContext)
...
8. **i18n**: ~10 NLS keys in package.nls.json + ~30 runtime keys in novacode.json
