...
│         │   │ • postprocessing     │  │ novacode-utils.ts  │        │
│         │   │ • tree-sitter queries│  └────────────────────┘        │
│         │   │ • LLM providers      │                                 │
│         │   └──────────────────────┘                                 │
│         │                                                            │
...
**LLM abstraction layer** — bridges autocomplete requests to the Nova Code API system.
...
- `../../api/providers` (`OpenRouterHandler`, `NovacodeOpenrouterHandler`)
...
8. **LLM providers** — `ILLM` interface with implementations for OpenAI, Mistral, OpenRouter, NovaCode, Mock.
...
Provides **autocomplete for the chat input text area** in the Nova Code webview. When the user is typing a message in the chat panel, this module suggests completions for natural language text.
...
- Filters out files matching security patterns (`isSecurityConcern`) and `.novacodeignore` rules
...
### [`novacode-utils.ts`](../utils/novacode-utils.ts)

- `checkNovacodeBalance(token, orgId)`: HTTP call to `/api/profile/balance` to verify positive balance.
...
| `RooIgnoreController`       | `src/core/ignore/`   | `.novacodeignore` pattern matching                                                |
