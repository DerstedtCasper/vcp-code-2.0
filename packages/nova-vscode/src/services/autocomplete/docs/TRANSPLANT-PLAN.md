...
- A **file ignore / access control abstraction** (to replicate `.novacodeignore` behavior and prevent sensitive files from being used).
...
This plan defines those interfaces and the minimum VS Code extension scaffolding required, without coupling the new extension to Nova Code’s current provider implementations.
...
In the current code, [`AutocompleteModel.reload()`](src/services/autocomplete/AutocompleteModel.ts:49) scans “profiles” from a settings manager and picks one (including special handling for the `novacode` provider).
...
Current Nova Code activates broadly:
...
Commands declared in Nova Code’s `package.json` (some are placeholders):
...
- `novacode.autocomplete.inline-completion.accepted` (acceptance callback)
...
- Register a `CodeActionsProvider` programmatically (as Nova Code does)
...
Nova Code does **not** contribute `configuration` settings; it stores everything in global state.
...
Most modifications are to replace Nova Code specific imports with the new interfaces.
...
- Ensure `setContext` keys are correctly set (notably `novacode.autocomplete.enableSmartInlineTaskKeybinding`, see [`investigation-vscode-integration.md`](src/services/autocomplete/docs/investigation-vscode-integration.md:145)).
...
- Uses internal Nova Code wrapper and webview provider.
...
Nova Code’s `package.json` command titles reference NLS keys like:
...
Runtime strings are accessed via `t()` from `src/i18n` (Nova Code).

Keys used live under namespace `novacode:autocomplete.*` (see JSON excerpt in [`investigation-vscode-integration.md`](src/services/autocomplete/docs/investigation-vscode-integration.md:287)).
...
3. Refactor `index.ts` entry point to accept dependency injection rather than Nova Code’s provider.
...
**Decision: Option A — Via CLI backend (Nova Gateway).**

FIM is postponed; only the **holefiller** (chat-completion-based) strategy will be used initially. The LLM provider will route completions through the Nova Gateway backend. The only supported model for now is `mistralai/codestral-2508`.

### 11.2 Provider & Model Selection

**Decision: Hardcoded to Nova Gateway + `mistralai/codestral-2508`.**
...
| Continuedev LLM adapters   | **Strip to minimum** — remove all adapters not needed for the Nova Gateway / holefiller path. The module uses `AutocompleteModel`, not the continuedev adapters directly, so most can be removed. |
...
**Decision: Wire into `@novacode/nova-i18n`.** Use the translations from `src/services/autocomplete/i18n/`. Keys may differ from what `nova-i18n` uses, so mapping is required. Discard translations for locales not present in `nova-i18n`.
...
**Decision: Deferred** — will be determined during implementation based on what works best with the existing `KiloConnectionService` / `NovaProvider` architecture.
